...
## How EdgeJS got red squiggles, typed render calls, and a language server
...
**The encouraging part:** AdonisJS v7 (2026) made end-to-end type safety its headline feature — codegen'd route types, type-checked `inertia.render` page props, a typed `urlFor` helper *available inside Edge templates*. Edge views were the one remaining gap: the pattern was already validated in-framework, it just had not been pointed at Edge yet.
...
So the gap was real and the demand documented. The "complete LSP from scratch" estimate was the thing to test — because the tooling landscape it was priced against no longer exists.
...
## Part 5: The bugs worth writing down
...
~40 commits and 185 tests later, five packages:
...
The original itch, revisited: it needed no changes to Edge itself, no fork, no runtime cost, and no "complete LSP from scratch" — Volar plus Edge's own lexer/parser plus roughly two thousand lines of generator glue. The from-scratch estimate simply predates Volar.js; the landscape changed underneath it.
