...
<Panel title="Not on npm yet">
  ...
  — or link the packages into your own project:

  ```sh
  # in the clone                        # in your project
  cd packages/check && bun link        bun link @edge-language-tools/check
  cd packages/codegen && bun link      bun link @edge-language-tools/codegen

  # npm equivalents
  cd packages/check && npm link        npm link @edge-language-tools/check
  ```
