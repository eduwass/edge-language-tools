---
title: The story
description: Where this came from — prior art, design decisions, and the bugs worth writing down.
---
...
```text
.edge templates + @types blocks        (the one thing you write)
        |
        v
@edge-language-tools/core              virtual TS + exact offset mappings
        |
        +--> Volar language server     squiggles in Zed / VS Code / Cursor
        +--> edge-check CLI            caret diagnostics, JSON output, CI exit codes
        +--> edge-codegen              generated templates.d.ts
                    |
                    v
             TypedEdge                 edge.render('profile', props) checked at compile time
```
...
```text
@form.input({ label: 'Email', requird: true })
        |  reverse mapping: form.input -> components/form/input.edge
        v
read the target's @types block
        |
        v
__component<Props>({ ...verbatim props... })
        |
        v
TS2353: 'requird' does not exist -- squiggle on the caller's typo
```
...
~38 commits and 185 tests later, five packages:
